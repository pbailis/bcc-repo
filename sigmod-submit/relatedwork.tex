...
Blazes~\cite{blazes} system detect non-monotonic, operations. This is
a useful \textit{liveness} guarantee---something good will
...
relax the requirement for deterministc outcomes. Commutative
Replicated Data Type (CRDT) objects~\cite{crdt} similarly ensure that,
once a database quiesces writes and all servers exchange writes, the
database will reflect all prior updates made to each CRDT. CRDTs are
accordingly useful in providing liveness via merging divergent
replicas on a per-item basis but do not maintain application-level
consistency.
